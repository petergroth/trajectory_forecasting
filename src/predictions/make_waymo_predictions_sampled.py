...
def make_sampled_predictions(path, config, n_steps=51, sequence_idx=0, seed=0):
    ...
    y_hat, y_target, mask, Sigma, pos_loc = regressor.sample_trajectories(batch, prediction_horizon=n_steps)
...
def main():
    ...
    for t in range(n_steps - 1):
        ...
        for agent in range(n_agents):
            ...
            ell = Ellipse(xy=loc[agent], width=w, height=h, angle=theta, color=agent_colors[agent], alpha=0.2)
            ax[1].add_artist(ell)
