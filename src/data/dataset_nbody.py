...
class OneStepTrainDataset(InMemoryDataset):
    ...
    def process(self):
        assert len(self.raw_file_names) == 1000
        data_list = []

        # Move through training samples and save all graphs separately
        for i, raw_path in enumerate(self.raw_file_names[:500]):
...
class SequentialTrainDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        for j, raw_path in enumerate(self.raw_file_names[:500]):
            full_arr = load_simulations(
                self.raw_dir + "/" + raw_path
            )  # [n_steps, n_nodes, 5]
            n_steps = full_arr.shape[0]
...
class SequentialValDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        for j, raw_path in enumerate(self.raw_file_names[500:750]):
            full_arr = load_simulations(
                self.raw_dir + "/" + raw_path
            )  # [n_steps, n_nodes, 5]
            n_steps = full_arr.shape[0]
...
class SequentialTestDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        for j, raw_path in enumerate(self.raw_file_names[750:]):
            full_arr = load_simulations(
                self.raw_dir + "/" + raw_path
            )  # [n_steps, n_nodes, 5]
            n_steps = full_arr.shape[0]
...
class OneStepNBodyDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        assert len(self.val_dataset) == len(self.test_dataset) == 250
...
class SequentialNBodyDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        self.test_dataset = SequentialTestDataset()
        assert len(self.val_dataset) == len(self.test_dataset) == 250
        assert len(self.train_dataset) == 500
