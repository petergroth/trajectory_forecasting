...
class ObjectiveGCN(object):
    ...
    def __call__(self, trial):
        ...
        training_horizon = trial.suggest_categorical("training_horizon", [12, 13, 14, 15, 25, 30, 40, 50, 70, 90])
        teacher_forcing_ratio = trial.suggest_float("teacher_forcing_ratio", low=0.0, high=0.3, step=0.05)
        min_dist = trial.suggest_float("min_dist", low=1.0, high=30.0, step=1.0)
        ...
        return val_total_lossa


class ObjectiveGAT(object):
    def __init__(self, config):
        self.config = config
    ...
    def __call__(self, trial):
        ...
        middle_gat = trial.suggest_categorical("middle_gat", [True, False])
