...
from torch_geometric.nn import GATConv, GCNConv, Sequential, GatedGraphConv, MessagePassing
...
class ConstantModel(nn.Module):
    def __init__(self):
        ...
        self.NormBlock = NormalisationBlock(normalise=False, node_features=5,
                                            edge_features=0)
...
class mlp_forward_model(nn.Module):
    ...
    def __init__(self, hidden_size: int = 64, node_features: int = 5, dropout: float = 0.0,
                 skip: bool = True, normalise: bool = True, aggregate: bool = False, **kwargs):
        ...
        self.GN1 = GraphNetworkBlock(
            node_model=node_mlp_1(
                hidden_size=hidden_size, node_features=node_features, dropout=dropout, edge_features=0
           )
        )
        ...
        self.GN2 = GraphNetworkBlock(
            node_model=node_mlp_out(
                hidden_size=hidden_size, node_features=GN2_node_input, dropout=dropout, edge_features=0
            )
        )

        self.NormBlock = NormalisationBlock(normalise=normalise, node_features=node_features,
                                            edge_features=0)
    ...
    def forward(self, x, edge_index, edge_attr, batch=None, u=None):
        ...
        # First block
        x_1, edge_attr_1, _ = self.GN1(x=x, edge_index=edge_index, edge_attr=edge_attr, u=u, batch=batch)
        ...
        # Second block
        out, _, _ = self.GN2(x=x_1, edge_index=edge_index, edge_attr=edge_attr_1, u=u, batch=batch)
        return out


class mlp_full_forward_model(nn.Module):
    # Forward model with edge update function
    def __init__(self, hidden_size: int = 64, node_features: int = 5, dropout: float = 0.0, edge_features: int = 0,
                 latent_edge_features: int = 0, skip: bool = True, normalise: bool = True, aggregate: bool = False):
        ...
        self.GN1 = GraphNetworkBlock(
            edge_model=edge_mlp_1(
                node_features=node_features, edge_features=edge_features, hidden_size=hidden_size, dropout=dropout,
                latent_edge_features=latent_edge_features
            ),
            node_model=node_mlp_1(
                hidden_size=hidden_size, node_features=node_features, dropout=dropout,
                edge_features=latent_edge_features
           )
        )
        GN2_node_input = node_features+hidden_size if skip else hidden_size
        GN2_edge_input = edge_features+latent_edge_features if skip else latent_edge_features

        self.GN2 = GraphNetworkBlock(
            edge_model=edge_mlp_1(
                node_features=GN2_node_input, edge_features=GN2_edge_input, hidden_size=hidden_size,
                dropout=dropout, latent_edge_features=latent_edge_features
            ),
            node_model=node_mlp_out(
                hidden_size=hidden_size, node_features=GN2_node_input, dropout=dropout,
                edge_features=latent_edge_features
            )
        )
...
class mpnn_forward_model(nn.Module):
    ...
    def __init__(self, hidden_size: int = 64, node_features: int = 5, dropout: float = 0.0, edge_features: int = 0,
                 latent_edge_features: int = 0, normalise: bool = True, rounds: int = 1, shared_params: bool = True,
                 **kwargs):
        ...
        if shared_params:
            ...
            self.processor = GraphNetworkBlock(
                edge_model=edge_mlp_latent(
                    node_features=hidden_size, hidden_size=hidden_size, dropout=dropout,
                    latent_edge_features=latent_edge_features
                ),
                node_model=node_mlp_latent(
                    hidden_size=hidden_size, node_features=hidden_size, dropout=dropout,
                    edge_features=latent_edge_features
               )
            )
        else:
        ...
        self.decoder = GraphNetworkBlock(
            edge_model=edge_mlp_1(
                node_features=hidden_size, edge_features=latent_edge_features, hidden_size=hidden_size,
                dropout=dropout, latent_edge_features=latent_edge_features
            ),
            node_model=node_mlp_out(
                hidden_size=hidden_size, node_features=hidden_size, dropout=dropout, edge_features=latent_edge_features
            )
        )
    ...
    def forward(self, x, edge_index, edge_attr, batch=None, u=None):
        ...
        x_latent_prev, edge_attr_latent_prev, _ = self.encoder(x=x, edge_index=edge_index, edge_attr=edge_attr, u=u,
                                                               batch=batch)
        # Perform message passing
        if self.shared_params:
            # Use same block if sharing parameters
            for _ in range(self.rounds):
                x_latent, edge_attr_latent, _ = self.processor(x=x_latent_prev, edge_index=edge_index,
                                                               edge_attr=edge_attr_latent_prev, u=u, batch=batch)
            ...
            for mp_block in self.processors:
                x_latent, edge_attr_latent, _ = mp_block(x=x_latent_prev, edge_index=edge_index,
                                                         edge_attr=edge_attr_latent_prev, u=u, batch=batch)
                x_latent_prev += x_latent
                edge_attr_latent_prev += edge_attr_latent

        # Decode output to produce delta dynamics
        out, _, _ = self.decoder(x=x_latent_prev, edge_index=edge_index, edge_attr=edge_attr_latent_prev,
                                 u=u, batch=batch)
...
class rnn_full_forward_model(nn.Module):
    # Forward model with edge update function
    def __init__(self, hidden_size: int = 64, node_features: int = 5, dropout: float = 0.0, edge_features: int = 0,
                 latent_edge_features: int = 0, skip: bool = True, normalise: bool = True, num_layers: int = 1,
                 rnn_size: int = 20):
        ...
        self.GN1 = GraphNetworkBlock(
            edge_model=edge_rnn_1(
                node_features=node_features, edge_features=edge_features, rnn_size=rnn_size, dropout=dropout,
        ...
        self.GN2 = GraphNetworkBlock(
            edge_model=edge_mlp_1(
                node_features=GN2_node_input, edge_features=0, hidden_size=hidden_size,
                dropout=dropout, latent_edge_features=GN2_edge_input
            ),
            node_model=node_mlp_out(
                hidden_size=hidden_size, node_features=GN2_node_input, dropout=dropout, edge_features=GN2_edge_input
            )
        )
    ...
    def forward(self, x, edge_index, edge_attr, batch=None, u=None, hidden=None):
        ...
        # First block
        x_1, edge_attr_1, _, hidden = self.GN1(x=x, edge_index=edge_index, edge_attr=edge_attr, u=u, batch=batch,
                                               hidden=hidden)
        # Concatenation of node and edge attributes
        x_1 = torch.cat([x, x_1], dim=1)  # [rnn_dim + node_features]
        edge_attr_1 = torch.cat([edge_attr, edge_attr_1], dim=1) # [rnn_dim + edge_features]
        # Second block
        out, _, _ = self.GN2(x=x_1, edge_index=edge_index, edge_attr=edge_attr_1, u=u, batch=batch)
        return out, hidden
...
class rnn_node_forward_model(nn.Module):
    ...
    def __init__(self, hidden_size: int = 64, node_features: int = 5, dropout: float = 0.0, edge_features: int = 0,
        ...
        self.GN1 = GraphNetworkBlock(
            ...
            node_model=node_rnn_1(
                node_features=node_features, edge_features=latent_edge_features, rnn_size=rnn_size, dropout=dropout,
                num_layers=num_layers
           )
        )
        GN2_node_input = rnn_size+node_features if skip else rnn_size
        GN2_edge_input = latent_edge_features+edge_features if skip else latent_edge_features
...
class conv_model(nn.Module):
    # Forward model without edge update function
    def __init__(self, hidden_size: int = 64, node_features: int = 5, dropout: float = 0.0,
                 skip: bool = True, normalise: bool = True, edge_features: int = 1, **kwargs):
        ...
        self.GN2 = GraphNetworkBlock(
            node_model=GCNConv(in_channels=GN2_node_input, out_channels=self.out_features)
        )
...
class GraphNetworkBlock(MetaLayer):
    ...
    def forward(
        ...
        if self.edge_model is not None:
            ...
            # if hidden is None:
            edge_attr = self.edge_model(src=x[row], dest=x[col], edge_attr=edge_attr, u=u, batch=btch,
        ...
        if self.node_model is not None:
            if hidden is None:
                ...
                x, node_hidden = self.node_model(x, edge_index, edge_attr, u, batch, hidden=hidden)
...
class mlp_baseline(nn.Module):
    # Forward model without edge update function
    def __init__(self, hidden_size: int = 64, node_features: int = 5, n_nodes: int = 10, normalise: bool = True,
                 dropout: float = 0.0, permute: bool = False, **kwargs):
        ...
        self.mlp_2 = nn.Sequential(
            nn.Linear(in_features=node_features*n_nodes + hidden_size, out_features=hidden_size),
            nn.Dropout(p=dropout),
            nn.ReLU(),
            nn.Linear(in_features=hidden_size, out_features=self.out_features*self.n_nodes)
        )
...
class rnn_baseline(nn.Module):
    # Forward model without edge update function
    def __init__(self, hidden_size: int = 64, node_features: int = 5, n_nodes: int = 10, normalise: bool = True,
                 dropout: float = 0.0, rnn_size: int = 20, num_layers: int = 1, **kwargs):
...
class rnn_graph_baseline(nn.Module):
    ...
    def __init__(self, hidden_size: int = 64, node_features: int = 5, n_nodes: int = 10, normalise: bool = True,
        ...
        self.mlp_2 = nn.Sequential(
            nn.Linear(in_features=rnn_size+n_nodes*node_features, out_features=hidden_size),
            nn.Dropout(p=dropout),
            nn.ReLU(),
            nn.Linear(in_features=hidden_size, out_features=self.n_nodes*self.out_features)
        )
...
class NormalisationBlock:
    def __init__(self, normalise: bool = True, node_features: int = 5, edge_features: int = 2,
    ...
    def update_in_normalisation(self, x, edge_attr=None):
        if self.normalise:
            ...
            self.node_in_std = torch.sqrt(
                ((self.node_in_squaresum / self.node_counter) - (self.node_in_sum / self.node_counter) ** 2)
            )
            ...
            if edge_attr is not None:
                ...
                self.edge_in_std = torch.sqrt(
                    ((self.edge_in_squaresum / self.edge_counter) - (self.edge_in_sum / self.edge_counter) ** 2)
                )
    ...
    def update_out_normalisation(self, x):
        if self.normalise:
            ...
            self.node_out_std = torch.sqrt(
                ((self.node_out_squaresum / self.node_counter) - (self.node_out_sum / self.node_counter) ** 2)
            )
