...
class OneStepModule(pl.LightningModule):
    def __init__(self, model_type, noise, lr: float, weight_decay: float, log_norm: bool, min_dist=None,
                 edge_weight: bool = False, self_loop: bool = True, undirected: bool = False, grav_attraction: bool
                 = False):
        super().__init__()
    ...
    def training_step(self, batch: Batch, batch_idx: int):
        ...
        if self.min_dist is not None:
            edge_index = torch_geometric.nn.radius_graph(
                x=x[:, :2], r=self.min_dist, batch=batch.batch, loop=self.self_loop, max_num_neighbors=30,
                flow='source_to_target')
            # 1 nearest neighbour to ensure connected graphs
            nn_edge_index = torch_geometric.nn.knn_graph(x=x[:, :2], k=1, batch=batch.batch)
            # Remove duplicates
            edge_index = torch_geometric.utils.coalesce(torch.cat((edge_index, nn_edge_index), dim=1))
        ...
        y_hat = self.model(x=x_nrm, edge_index=edge_index, edge_attr=edge_attr_nrm, batch=batch.batch)
        ...
        self.log("train_total_loss", (pos_loss+vel_loss)/2, on_step=True, on_epoch=True)
    ...
    def validation_step(self, batch: Batch, batch_idx: int):
        ...
        for t in range(11):
            ...
            # Obtain normalised predicted delta dynamics
            x = self.model(x=x, edge_index=edge_index, edge_attr=edge_attr, batch=batch.batch)
            # Renormalise output dynamics
            x = self.model.NormBlock.out_renormalise(x)
            # Add deltas to input graph
            predicted_graph = torch.cat((batch.x[:, t, :4] + x, static_features), dim=-1)  # [n_nodes, n_features]
        ...
        for t in range(11, 90):
            ...
            x = self.model.NormBlock.out_renormalise(x)
            # Add deltas to input graph
            predicted_graph = torch.cat((predicted_graph[:, :4] + x, static_features), dim=-1)  # [n_nodes, n_features]
        ...
        if self.log_norm:
            self.log("in_std", {f"in_std_{i}": std for i, std in enumerate(self.model.in_std)})
            self.log("in_mean", {f"in_mean_{i}": mean for i, mean in enumerate(self.model.in_mean)})
            self.log("out_std", {f"out_std_{i}": std for i, std in enumerate(self.model.out_std)})
            self.log("out_mean", {f"out_mean_{i}": mean for i, mean in enumerate(self.model.out_mean)})
    ...
    def predict_step(self, batch, batch_idx=None):
        ...
        for t in range(11):
            ...
            x = self.model.NormBlock.out_renormalise(x)
            # Add deltas to input graph
            predicted_graph = torch.cat((batch.x[:, t, :4] + x, static_features), dim=-1)
            # Save predictions
...
class SequentialModule(pl.LightningModule):
    def __init__(self, model_type, lr: float, weight_decay: float, log_norm: bool, noise=None,
                 teacher_forcing: bool = False, teacher_forcing_ratio: float = 0.3, min_dist=None,
                 edge_weight: bool = False, self_loop: bool = True, undirected: bool = False,
                 rnn_type: str = 'GRU', grav_attraction: bool = False):
        super().__init__()
    ...
    def training_step(self, batch: Batch, batch_idx: int):
        ...
        y_target_abs = (torch.roll(x[:, :, :4], (0, -1, 0), (0, 1, 2)) - x[:, :, :4])[:, :-1, :]
        ...
        for t in range(11):
            ...
            if self.min_dist is not None:
                edge_index = torch_geometric.nn.radius_graph(
                    x=x_t[:, :2], r=self.min_dist, batch=batch.batch, loop=self.self_loop, max_num_neighbors=30,
                    flow='source_to_target')
                # 1 nearest neighbour to ensure connected graphs
                nn_edge_index = torch_geometric.nn.knn_graph(x=x_t[:, :2], k=1, batch=batch.batch)
                # Remove duplicates
                edge_index = torch_geometric.utils.coalesce(torch.cat((edge_index, nn_edge_index), dim=1))
                self.log("average_num_edges", edge_index.shape[1] / batch.num_graphs)
            ...
            if edge_attr is None:
                ...
                self.model.NormBlock.update_in_normalisation(x_t.detach().clone(), edge_attr.detach().clone())
            ...
            # Obtain normalised predicted delta dynamics
            y_pred_t, h = self.model(x=x_t_nrm, edge_index=edge_index, edge_attr=edge_attr_nrm, batch=batch.batch,
                                     hidden=h)
        ...
        use_groundtruth = (torch.distributions.uniform.Uniform(0, 1).sample() < self.teacher_forcing_ratio) \
            if self.teacher_forcing else False
        ...
        # Compute and log loss
        fde_loss = self.train_fde_loss(y_predictions[:, -1, :2], y_target_nrm[:, -1, :2])
        ...
        self.log("train_vel_loss", vel_loss, on_step=True, on_epoch=True)
        self.log("train_total_loss", (vel_loss+ade_loss)/2, on_step=True, on_epoch=True)
    ...
    def validation_step(self, batch: Batch, batch_idx: int):
        ...
        for t in range(11):
            ...
            x, edge_attr = self.model.NormBlock.in_normalise(x, edge_attr)
            # Obtain normalised predicted delta dynamics
            x, h = self.model(x=x, edge_index=edge_index, edge_attr=edge_attr, batch=batch.batch, hidden=h)
            # Renormalise output dynamics
...
class ConstantBaselineModule(pl.LightningModule):
    ...
    def predict_step(self, batch, batch_idx=None):
        ...
        for t in range(1, 11):
            y_hat[t, :, :] = batch.x[:, t, :] + (batch.x[:, t, :] - batch.x[:, t-1, :])
...
class ConstantPhysicalBaselineModule(pl.LightningModule):
    ...
    def validation_step(self, batch: Batch, batch_idx: int):
        ...
        predicted_graph = torch.cat((predicted_pos, last_vel, static_features.unsqueeze(1)), dim=1)
        # Save first prediction and target
        y_hat[0, :, :] = predicted_graph[:, :4]
        y_target[0, :, :] = batch.x[:, 11, :4]
...
if __name__ == '__main__':
    ...
    # Setup logging
    wandb_logger = WandbLogger(entity='petergroth', config=config,
                               **config['logger'])
    ...
    if config["misc"]["checkpoint"]:
        ...
        checkpoint_callback = pl.callbacks.ModelCheckpoint(
            monitor="val_ade_loss", dirpath='checkpoints',
            filename=config["logger"]["version"]+'-{epoch}-{val_ade_loss:.2f}'
        )
        # Create trainer, fit, and validate
        trainer = pl.Trainer(logger=wandb_logger, **config['trainer'], callbacks=[checkpoint_callback])
    else:
        # Create trainer, fit, and validate
    ...
    if config["misc"]["continue_training"] is not None:
        ...
        trainer = pl.Trainer(
            logger=wandb_logger, **config['trainer'],
